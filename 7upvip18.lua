local SelNum, Package, Table, TabNum, Enable_Always_Showup, Always_Showup_Word, Add_Exit_Buttom, Menu, A, B, C, D, E
-- ^^^^^^^^^^^ Leave those on top,put value on script only.(Can't across another script to prevent error)
--[[ Add your code like function here: ]]-----

--[[

		~~~Alert Gui V3 By Sam(noBlack)~~~
	Please Leave this message on here when you use it, its the credit 4 me. 
	Content me on Discord: 
			DESPAC1T0#6969
	If you have any problem when using them, 
	thank you 4 using!
	*Go Down and DIY(Do It YourSelf)*
...
--After setting up table,go down and
--change the value if you want.
function Startup()
--Edit the Value at your own
--When you need

	Enable_Always_Showup = false
--Default false.If you like you can enable it.
	Always_Showup_Word = ""
--Default empty,you can add something on it WHEN YOU ENABLE SHOWUP WORD
	Add_Exit_Buttom = false
--Default true,false can be added.It will add a Exit Buttom on your menu and you dont have to make one at your own : )
	SelNum = 1
-- Selected Number. When Menu Starts,which string is display at first

--DON'T TOUCH
	CreateTabNum(DONT_TOUCH) 
--Create the menu
end

--DIY end.Now run script and see is there any error.

--gg.toast("Scroller V4")
--Show WaterMark on start, you can add "--" or delete that to remove it.
 function CreateTabNum()
	TabNum = 0
	for _ in pairs(Table) do
		TabNum = TabNum + 1 
		end
		if Add_Exit_Buttom == true then
			Table[TabNum + 1] = "[■]Exit[■]"
			end
end
...
--Main Core
Startup()
--Run important value (See in function Startup() )
if Enable_Always_Showup == true then 
Always_Showup_Word = Always_Showup_Word.."\n"
else
Always_Showup_Word = ""
end
--Add a "Enter" on it.
...
repeat
	if gg.isVisible(true) then
		gg.setVisible(false)
	Menu = gg.alert(Always_Showup_Word..Table[SelNum],"[>]","[<]","·Select·")
	if Menu == 1 then
		SelNum = SelNum + 1
		end
	if Menu == 2 then
	...
	if Add_Exit_Buttom == true then
		if SelNum == TabNum + 2 then
			SelNum = 1
			end
		end
	if Add_Exit_Buttom ~= true then
		if SelNum == TabNum + 1 then
			SelNum = 1
			end
		end
	if Add_Exit_Buttom == true then
